...
export class King implements IChessPiece {
  ...
  verifyCell( position : number, isEmpty : boolean ) {

    let canMove = false;

    if( this.currentPosition + 1 === position && isEmpty ) canMove = true;
    ...
    if( this.currentPosition + 8 === position && isEmpty ) canMove = true;
